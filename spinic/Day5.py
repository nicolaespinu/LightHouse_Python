# Challenge
# Dot's okay with paying a bit of a surplus for convenience, but they don't want to go broke buying dustpans
# and glass cleaner. Help them figure out which items cost over 10% more at the nearby store,
...
for i in range(len(cleaningsupplies_list)):
    ...
    if ((country_price[i] - city_price[i]) / city_price[i]) > 0.1:
        exp_items.append(cleaningsupplies_list[i])
